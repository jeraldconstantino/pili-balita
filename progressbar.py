...
_DEFAULT_THICKNESS = 4
_DEFAULT_CAP_STYLE = "round"
_DEFAULT_PRECISION = 10
_DEFAULT_PROGRESS_COLOUR = (127/255, 96/255, 7/255, 255/255)
_DEFAULT_BACKGROUND_COLOUR = (0.26, 0.26, 0.26, 0.5)
_DEFAULT_MAX_PROGRESS = 100
_DEFAULT_MIN_PROGRESS = 0
_DEFAULT_WIDGET_SIZE = 110
_DEFAULT_TEXT_LABEL = Label(text="{}%", font_size=35, color=(0,0,0,0))
...
class CircularProgressBar(Widget):
    ...
    def thickness(self, value):
        if type(value) != int:
            raise TypeError(
                "Circular bar thickness only accepts an integer value, not {}!".format(
                    type(value)
                )
            )
        elif value <= 0:
            raise ValueError(
                "Circular bar thickness must be a positive integer, not {}!".format(
                    value
                )
            )
        else:
    ...
    def cap_precision(self, value: int):
        if type(value) != int:
            ...
            raise ValueError(
                "Circular bar cap precision must be a positive integer, not {}!".format(
                    value
                )
            )
        else:
    ...
    def max(self, value: int):
        if type(value) != int:
            raise TypeError(
                "Maximum progress only accepts an integer value, not {}!".format(
                    type(value)
                )
            )
    ...
    def min(self, value: int):
        if type(value) != int:
            raise TypeError(
                "Minimum progress only accepts an integer value, not {}!".format(
                    type(value)
                )
            )
    ...
    def widget_size(self, value: int):
        if type(value) != int:
            raise TypeError(
                "Size of this widget must be an integer value, not {}!".format(
                    type(value)
                )
            )
        elif value <= 0:
